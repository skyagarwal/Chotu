...
export class UserProfileEnrichmentService {
  ...
  private async initializePools() {
    ...
    const mysqlHost = process.env.MYSQL_HOST || '103.86.176.59';
  ...
  async analyzeOrderHistory(userId: number): Promise<OrderPatterns> {
    ...
    try {
      ...
      const [favoriteItems]: any = await this.mysqlPool.query(
        `SELECT od.item_id, od.food_details, COUNT(*) as order_count
         FROM order_details od
         JOIN orders o ON od.order_id = o.id
         WHERE o.user_id = ? AND o.order_status NOT IN ('canceled', 'failed')
         GROUP BY od.item_id, od.food_details
         ORDER BY order_count DESC
         LIMIT 10`,
        [userId]
      );
      ...
      patterns.favoriteItems = favoriteItems.map((item: any) => {
        let details: any = {};
        try {
          details = typeof item.food_details === 'string' 
            ? JSON.parse(item.food_details) 
            : item.food_details || {};
        } catch {}

        return {
          itemId: item.item_id,
          itemName: details.name || `Item ${item.item_id}`,
